...
def pricingProblem(dualCommodities, dualArcs): # For the moment, return the path corresponding to the most violated constraint
    ...
    for i in range(nCommodities): #Solve the shortest path problem (with updated length) for each commodity
        ...
        try:
            ...
            if(model.solution.get_objective_value() < 0): # Once we get a negative reduced cost, return the corresponding path
                reducedCost = model.solution.get_objective_value()
                bestPath = model.solution.get_values()
                forCommodity = i
                
                tempCost = 0 #Compute the cost of "bestPath"
                for j in range(nArcs):
                    if(bestPath[j] == 1):
                        tempCost += arc[j][2]
...
t_time_loading_data = time.time()
 ...
 # Get the initial set of variable through Dijkstra and "< limit*shortest_path " method
pathsK = getInitSet()
        ...
        for i in range(nCommodities):
            ...
            for j in range(len(pathsK[i])):
                ...
                print("\t", solution[count]*commodities[i][2] ,"quantity of commodity n°", i ,"on path", node[commodities[i][0]][0],''
                      + ' '.join([node[arc[k][1]-1][0] for k in indices]) + ". Length path : ", pathsK[i][j][nArcs] )
                count += 1
        print("\nTotal cost = ", obj_Function)
